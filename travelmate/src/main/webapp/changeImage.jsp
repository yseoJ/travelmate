...
<%
	...
	try{
		if(-1<request.getContentType().indexOf("multipart/from-data")){
			mr = new MultipartRequest(request, application.getRealPath("/image"), 1024*1024*1024, "utf-8", new DefaultFileRenamePolicy());
	...
	sql = String.format("SELECT IMG FROM MEMB_IMG WHERE MEMB_ID = '%s'", memb_id);
...
<body style="line-height: 100%">
	...
	<br><h2 style="text-align: center">프로필 수정</h2>
	<hr> 
	<div style="margin: 0 auto; text-align: center;">
			<form action="form_result.jsp" method="post" enctype="multipart/form-data">
				...
				<button type="button" onClick="onChange();" style="line-height:25px; display: inline-block; border-radius: 6px; background-color: rgba(13, 45, 132); height: 25px;">수정</button>
			</form>
	...
	<script type="text/javascript">
		function onChange() {
			...
			var fileName = mr.getFilesystemName("profile");
	        ...
	        console.log("fileName: " + fileName);
  	  		...
  	  		$.ajax({ url: "changeImageFinish.jsp", // 클라이언트가 HTTP 요청을 보낼 서버의 URL 주소 
	  	  		data:{ 
	  	  				id: i,
	  	  				file: fileName
